...
class Trainer:
    def __init__(self, config, allow_distributed, compile_model=False, device='cuda',save_final_model=True):
        self.config = config
        ...
        for step in pbar:
            ...
            if (step + 1) % eval_every == 0:
                if rank is None or rank == 0:
                    ...
                    model.train()
        if self.save_final_model and (rank is None or rank == 0):
            final_path = os.path.join(self.config.log_dir, 'final_model')
...
def create_parser():
    ...
    parser.add_argument('--save_final_model', type=bool, default=True,
                    help='Whether to save the final model after training.')
